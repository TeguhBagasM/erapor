...
<head>
    ...
    <title>Rapor Kelas {{ $kelas->nama_kelas }} — {{ $tahunAjaran->tahun_ajaran }}</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media screen {
            ...
            .rapor-page { max-width: 210mm; margin: 0 auto 30px; background: #fff; padding: 15mm 20mm; box-shadow: 0 2px 12px rgba(0,0,0,.15); }
            .rapor-page:first-of-type { margin-top: 60px; }
        }
        ...
        .page-indicator { text-align: right; font-size: 9pt; color: #999; margin-bottom: 5px; }
    </style>
</head>
<body>
    <div class="print-toolbar no-print">
        <button onclick="window.print()"><i class="fas fa-print"></i> Cetak Semua ({{ $siswaList->count() }} siswa)</button>
        <a href="javascript:history.back()">&larr; Kembali</a>
        <span style="font-size:13px;opacity:.7;margin-left:auto;">
            {{ $kelas->nama_kelas }} — {{ $tahunAjaran->tahun_ajaran }} {{ ucfirst($tahunAjaran->semester) }}
        </span>
    </div>

    @foreach($siswaList as $siswaIndex => $siswa)
        @php
            $nilaiSiswa = isset($allNilai[$siswa->id]) ? $allNilai[$siswa->id]->keyBy('mata_pelajaran_id') : collect();
            $totalNilai = 0;
            $countNilai = 0;
        @endphp

        <div class="rapor-page">
            <div class="page-indicator">Halaman {{ $siswaIndex + 1 }} dari {{ $siswaList->count() }}</div>
